...
from .app import add_companion_arguments, command_companion
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--gui", action="store_true", help="run the GTK3 TokenWatch companion window")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8765)
    parser.add_argument("--interval", type=float, default=1.0)
    add_companion_arguments(parser)
    args = parser.parse_args(argv)

    companion_result = command_companion(args, args.root)
